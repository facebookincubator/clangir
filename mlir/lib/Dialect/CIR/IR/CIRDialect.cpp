...
static LogicalResult verify(GlobalOp op) {
  ...
  if (op.initial_value().hasValue()) {
    if (checkConstantTypes(op.getOperation(), op.sym_type(),
                           *op.initial_value())
            .failed())
      return failure();
  }
...
static void print(cir::FuncOp op, OpAsmPrinter &p) {
  ...
  function_like_impl::printFunctionSignature(p, op, fnType.getInputs(),
                                             /*isVariadic=*/false,
                                             fnType.getResults());
  function_like_impl::printFunctionAttributes(p, op, fnType.getNumInputs(),
